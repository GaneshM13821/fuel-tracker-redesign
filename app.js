...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Wire export button (add one in your HTML: <button id="exportAllCSV">Export All</button>)
  const exportBtn = document.getElementById("exportAllCSV");
  if (exportBtn) {
    exportBtn.addEventListener("click", exportAllXLSX);
  }
...
function exportAllXLSX() {
  ...
  fuelRecords.forEach((r, i) => {
    const row = i + 1;
    wsFuel[`D${row+1}`].z = "0.0";       // Distance
    wsFuel[`E${row+1}`].z = "0.00";      // Fuel
    wsFuel[`F${row+1}`].z = "₹ #,##0.00"; // Cost
  });
